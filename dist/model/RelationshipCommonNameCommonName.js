"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports["default"] = void 0;

var _ApiClient = _interopRequireDefault(require("../ApiClient"));

var _RelationshipMemberTlsDomain = _interopRequireDefault(require("./RelationshipMemberTlsDomain"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { "default": obj }; }

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function _defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } }

function _createClass(Constructor, protoProps, staticProps) { if (protoProps) _defineProperties(Constructor.prototype, protoProps); if (staticProps) _defineProperties(Constructor, staticProps); return Constructor; }

/**
 * The RelationshipCommonNameCommonName model module.
 * @module model/RelationshipCommonNameCommonName
 * @version 3.0.0-alpha1
 */
var RelationshipCommonNameCommonName = /*#__PURE__*/function () {
  /**
   * Constructs a new <code>RelationshipCommonNameCommonName</code>.
   * @alias module:model/RelationshipCommonNameCommonName
   */
  function RelationshipCommonNameCommonName() {
    _classCallCheck(this, RelationshipCommonNameCommonName);

    RelationshipCommonNameCommonName.initialize(this);
  }
  /**
   * Initializes the fields of this object.
   * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
   * Only for internal use.
   */


  _createClass(RelationshipCommonNameCommonName, null, [{
    key: "initialize",
    value: function initialize(obj) {}
    /**
     * Constructs a <code>RelationshipCommonNameCommonName</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/RelationshipCommonNameCommonName} obj Optional instance to populate.
     * @return {module:model/RelationshipCommonNameCommonName} The populated <code>RelationshipCommonNameCommonName</code> instance.
     */

  }, {
    key: "constructFromObject",
    value: function constructFromObject(data, obj) {
      if (data) {
        obj = obj || new RelationshipCommonNameCommonName();

        if (data.hasOwnProperty('data')) {
          obj['data'] = _ApiClient["default"].convertToType(data['data'], [_RelationshipMemberTlsDomain["default"]]);
        }
      }

      return obj;
    }
  }]);

  return RelationshipCommonNameCommonName;
}();
/**
 * @member {Array.<module:model/RelationshipMemberTlsDomain>} data
 */


RelationshipCommonNameCommonName.prototype['data'] = undefined;
var _default = RelationshipCommonNameCommonName;
exports["default"] = _default;