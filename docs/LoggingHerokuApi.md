...
```javascript
createLogHeroku({ service_id, version_id, [name, ][placement, ][response_condition, ][format, ][format_version, ][token, ][url] })
```
...
```javascript
updateLogHeroku({ service_id, version_id, logging_heroku_name, [name, ][placement, ][response_condition, ][format, ][format_version, ][token, ][url] })
```
