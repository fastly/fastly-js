...
```javascript
createLogHoneycomb({ service_id, version_id, [name, ][placement, ][response_condition, ][format, ][format_version, ][dataset, ][token] })
```
...
```javascript
updateLogHoneycomb({ service_id, version_id, logging_honeycomb_name, [name, ][placement, ][response_condition, ][format, ][format_version, ][dataset, ][token] })
```
